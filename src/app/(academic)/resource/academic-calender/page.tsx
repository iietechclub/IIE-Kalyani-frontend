...
import { fetchAcademicCalendarPageData } from "@/dal/academic-calendar";
import { urlFromBackend } from "@/lib/utils";

export default async function AcademicCalendar() {
  const { session, calendar_pdf } = await fetchAcademicCalendarPageData();
  const pdfHref = urlFromBackend(calendar_pdf.url);

  // Main Layout: Header (left) + PDF Viewer (right)
  return (
    <main // biome-ignore lint/suspicious/noExplicitAny: not important
      style={{ ["--iie-primary" as any]: "#c53030" }}
      className="flex- mx-auto h-full max-w-7xl items-stretch space-y-4 bg-gray-50 px-4 py-12 text-gray-900 sm:space-y-6"
    >
      {/* --- LEFT: Hero Section --- */}
      <section className="flex flex-wrap items-center justify-between rounded-xl bg-white- p-6 shadow-sm-">
        <div className="max-w-180">
          ...
          <h1 className="mb-3 font-bold text-3xl md:text-4xl">
            Academic Calendar{" "}
            <span className="text-(--iie-primary)">{session}</span>
          </h1>
