...
const heroImage = "https://images.unsplash.com/photo-1658235081452-c2ded30b8d9f?q=80&w=1114&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D";
...
function TestimonialsCarousel({ members }: { members: AlumniMember[] }) {
  // We'll build a sliding window like in your example
  const [currentView, setCurrentView] = useState(0);
  const totalViews = Math.max(1, members.length - 2); // show 3 cards at once; if <=3 then 1 view
      ...
      >
        <div className="flex items-stretch justify-center gap-4 md:gap-6">
          {[0, 1, 2].map((offset) => {
            ...
            if (idx >= members.length) return null;
            const t = members[idx];

            return (
              <AnimatePresence
                key={`${t.documentId}-${currentView}`}
                mode="wait"
              >
                <MotionDiv
                  layout
                ...
                >
                  <article className="group h-full overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-xl">
                    ...
                    <div className="relative flex flex-col p-4 md:p-5">
                      ...
                      {/* animated accent bar */}
                      {/* <div className="inset-x-0 bottom-0 h-1 scale-x-95 rounded-full bg-linear-to-r from-rose-300 to-rose-600 transition-transform duration-300 group-hover:scale-x-100" /> */}
                    </div>
                  </article>
                </MotionDiv>
              </AnimatePresence>
...
export default function AlumniRelationPageClient(data: AlumniRelationPageData) {
  ...
  return (
    <main className="min-h-screen bg-neutral-50">
      ...
      <header className="relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 pt-28 pb-26">
          <MotionDiv
      ...
      <div className="container mx-auto px-4">
        ...
        <section className="mb-10">
          <TestimonialsCarousel members={data.alumni_members} />
        </section>
